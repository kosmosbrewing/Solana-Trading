...
| Hard stop | `wallet < 0.6 SOL` 에 도달하면 모든 lane halt (current operating floor) |
...
| Live canary — Stage 1 (Safety Pass, 48h) | Wallet truth drift, survival pass rate, 0.6 floor 무위반 | hard guardrail 무사고 | stage gate |
...
| Guard | 현 값 | 역할 |
|-------|-------|------|
| Wallet Stop | `wallet_sol < 0.6` | 전 lane halt |
...
- Wallet Stop Guard `< 0.6 SOL` → 전 lane halt
...
- Wallet Stop Guard 0.6 SOL 작동 확인 필수.
