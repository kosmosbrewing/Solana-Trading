...
- 현재 active execution 기준 문서는 [`docs/exec-plans/active/20260503_BACKLOG.md`](./docs/exec-plans/active/20260503_BACKLOG.md)다. [`docs/exec-plans/active/1sol-to-100sol.md`](./docs/exec-plans/active/1sol-to-100sol.md)는 historical plan 으로만 본다.
...
#### `sync-vps-data.sh` 동작 (2026-04-16 강화 / 2026-05-19 promotion gatekeeper 추가)
...
- **자동 smart-v3-evidence-report (2026-05-03 / 2026-05-05 posterior join / 2026-05-06 MAE diagnostics / 2026-05-07 live-block audit / 2026-05-08 MFE floor / 2026-05-11 strategy no-paper-fallback)**: sync 직후 `smart-v3-paper-trades.jsonl`, `smart-v3-live-trades.jsonl`, shared `trade-markouts.jsonl`, 선택적 `data/research/kol-transfers.jsonl` 로 smart-v3 cohort verdict + KOL transfer posterior 진단 생성 → `reports/smart-v3-evidence-YYYY-MM-DD.md/json`. T+ verdict coverage 는 close `positionId × anchorType × horizon` 기준이며, Closed Trades W/L 은 copyable/wallet-first 로 계산하고 token-only W/L 은 별도 표시한다. Closed Trades 는 `smart_v3_mae_fast_fail`, recovery-hold, MFE floor-exit/stage counts, pre-T1 MFE band counts 도 함께 표시한다. Live-canary strategy rejects are summarized by `smartV3LiveBlockReason`, `smartV3LiveBlockFlags`, and `paperWouldEnter=false`; they do not create main paper close rows. Jupiter/RPC API 0건 (file-only) — default ON.
...
- **자동 rotation-promotion-candidates-report (2026-05-19)**: sync 직후 `rotation-v1-paper-trades.jsonl`/aggregate fallback 에서 24h/7d promotion bridge 후보를 분리한다. Raw paper headline 이 아니라 comparable role, candidate/decision/hash, route proof, cost-aware, wallet-stress, parent-child same-decision bridge 를 따로 보며 `reports/rotation-promotion-candidates-24h-YYYY-MM-DD.md/json`, `reports/rotation-promotion-candidates-7d-YYYY-MM-DD.md/json` 을 생성한다. Jupiter/RPC API 0건 (file-only) — default ON.
- **자동 rotation-promotion-gatekeeper (2026-05-19)**: 24h/7d candidate JSON을 읽어 `READY` / `WAIT` / `REJECT` 를 판정하고 `reports/rotation-promotion-gatekeeper-YYYY-MM-DD.md/json` 을 생성한다. `READY`여도 live 자동 활성화는 절대 하지 않고, 수동 tiny micro-canary review 후보로만 표시한다. Readiness 추세는 `data/research/rotation-promotion-readiness-history.jsonl` 에 누적한다. 현재 기본 micro-canary envelope 는 floor `0.6 SOL`, sleeve loss cap `0.02 SOL`, max close `30`, min active days `7`, max ticket `0.002 SOL` 이다.
- **자동 rotation-promotion-readiness-trend (2026-05-19)**: sync 직후 readiness history 를 읽어 최근 bridge 증거가 `IMPROVING` / `FLAT` / `DETERIORATING` 중 어디인지 판정한다. 출력은 `reports/rotation-promotion-readiness-trend-YYYY-MM-DD.md/json` 이며, gatekeeper 와 마찬가지로 file-only report 다.
- **자동 capitulation-rebound-report (2026-05-09 / 2026-05-11 RR sidecar 포함)**: sync 직후 `capitulation-rebound-paper-trades.jsonl`, shared `trade-markouts.jsonl`, `missed-alpha.jsonl` 로 strict `kol_hunter_capitulation_rebound_v1` 과 RR `kol_hunter_capitulation_rebound_rr_v1` paper-only rebound close / T+15/30/60/180/300/1800 / no-trade counterfactual 진단 생성 → `reports/capitulation-rebound-YYYY-MM-DD.md/json`. Jupiter/RPC API 0건 (file-only) — default ON.
...
- **환경변수 주의**: smart-v3 evidence/rotation/capitulation/KOL transfer posterior/promotion gatekeeper report 추가는 운영 `.env` 변경이 필요 없다. `SKIP_KOL_TRANSFER_REPORT`, `KOL_TRANSFER_REPORT_SINCE`, `KOL_TRANSFER_INPUT`, `SKIP_SMART_V3_EVIDENCE_REPORT`, `SMART_V3_EVIDENCE_ROUND_TRIP_COST_PCT`, `SKIP_ROTATION_PROMOTION_CANDIDATES_REPORT`, `SKIP_ROTATION_PROMOTION_GATEKEEPER_REPORT`, `SKIP_ROTATION_PROMOTION_TREND_REPORT`, `ROTATION_PROMOTION_CANDIDATES_WINDOWS_HOURS`, `SKIP_CAPITULATION_REPORT`, `CAPITULATION_REPORT_ROUND_TRIP_COST_PCT` 는 sync/report-only shell knob 이며 runtime 전략 환경변수가 아니다. Smart-v3 MAE fast-fail / recovery-hold / MFE floor 와 live strict-quality/pre-entry-sell/combo-decay/KOL-fill-advantage strategy gates 는 runtime knob 이 추가됐지만 default-on 안전값이 있어 운영 `.env` override 는 필수가 아니다. Live-canary strategy rejects now emit `SMART_V3_STRATEGY_NO_PAPER_FALLBACK`; execution-safety fallbacks can still open paper rows for audit. Combo decay 는 entry-time fresh KOL key 를 기준으로 primary paper+live close 를 함께 보며, shadow arm 은 학습 대상에서 제외한다.
...
- Rotation canary 운영 의도는 `KOL_HUNTER_ROTATION_V1_LIVE_ENABLED=false` + `KOL_HUNTER_ROTATION_CHASE_TOPUP_LIVE_CANARY_ENABLED=false` + explicit allowlist `KOL_HUNTER_LIVE_CANARY_ARMS=...,rotation_underfill_exit_flow_v1` 이다. Allowlist 가 비어있지 않으면 legacy `KOL_HUNTER_ROTATION_UNDERFILL_LIVE_CANARY_ENABLED` / `KOL_HUNTER_ROTATION_UNDERFILL_LIVE_EXIT_FLOW_ENABLED` 는 source of truth 가 아니므로 false 로 둔다. 전체 rotation live를 열지 않고, chase-topup은 paper-only로 둔다. Underfill은 paper trigger 기준과 live trigger 기준을 맞추되 Real Asset Guard/live canary gate는 유지한다.
...
SKIP_ROTATION_PROMOTION_CANDIDATES_REPORT=true bash scripts/sync-vps-data.sh
SKIP_ROTATION_PROMOTION_GATEKEEPER_REPORT=true bash scripts/sync-vps-data.sh
SKIP_ROTATION_PROMOTION_TREND_REPORT=true bash scripts/sync-vps-data.sh
ROTATION_PROMOTION_CANDIDATES_WINDOWS_HOURS="24 168" bash scripts/sync-vps-data.sh
