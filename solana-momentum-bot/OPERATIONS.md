...
- **자동 smart-v3-evidence-report (2026-05-03 / 2026-05-05 posterior join / 2026-05-06 MAE diagnostics)**: sync 직후 `smart-v3-paper-trades.jsonl`, `smart-v3-live-trades.jsonl`, shared `trade-markouts.jsonl`, 선택적 `data/research/kol-transfers.jsonl` 로 smart-v3 cohort verdict + KOL transfer posterior 진단 생성 → `reports/smart-v3-evidence-YYYY-MM-DD.md/json`. T+ verdict coverage 는 close `positionId × anchorType × horizon` 기준이며, Closed Trades W/L 은 copyable/wallet-first 로 계산하고 token-only W/L 은 별도 표시한다. Closed Trades 는 `smart_v3_mae_fast_fail`, recovery-hold, pre-T1 MFE band counts 도 함께 표시한다. Jupiter/RPC API 0건 (file-only) — default ON.
