...
### Cupsey Lane Order Shape (Path A — 2026-04-11)
...
PROBE window = 45s
PROBE → WINNER: MFE ≥ +0.05%
PROBE → REJECT: MAE ≤ -0.8% OR 45s timeout
WINNER trailing = 1.5%
WINNER max hold = 8min
WINNER breakeven = entry + 0.1%
Max concurrent = 3 (config: cupseyMaxConcurrent)
```
...
> 2026-04-12: bootstrap_10s **signal-only** (executionRrReject=99.0) + **Cupsey Lane primary** (STALK→PROBE→WINNER, 0.01 SOL, 50-trade edge 검증 중) + KOL Wallet Tracker. Paper PnL에 왕복 비용 차감 도입. Strategy A/C dormant, D sandbox only.
