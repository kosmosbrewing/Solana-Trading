...
| Realtime bootstrap `volumeMcapSpikeTrigger` | **active default** | 10초봉 volume+buyRatio 2-gate 트리거 (`strategy=volume_spike`로 기록) |
...
- runtime artifact에서 bootstrap signal은 `strategy=volume_spike`로 기록되므로, 5분봉 Strategy A와 동일 의미로 읽으면 안 된다.
...
이 표는 현재 VPS 운영값 기준이다.
code default와 다른 항목은 비고에 함께 적는다.

| 키 | VPS 현재값 | 비고 |
|---|---|---|
| `REALTIME_TRIGGER_MODE` | `bootstrap` | bootstrap / core |
| `REALTIME_VOLUME_SURGE_MULTIPLIER` | `1.8` | 4/4 replay stable baseline, code default `3.0` |
| `REALTIME_VOLUME_SURGE_LOOKBACK` | `20` | lookback 30보다 우세 |
| `REALTIME_BOOTSTRAP_MIN_BUY_RATIO` | `0.60` | bootstrap 전용 soft filter, code default `0.55` |
| `REALTIME_CONFIRM_MIN_BARS` | `3` | core 전용 |
| `REALTIME_CONFIRM_MIN_CHANGE_PCT` | `0.02` | core 전용 |
| `REALTIME_COOLDOWN_SEC` | `300` | |
| `VOLUME_SPIKE_MULTIPLIER` | `2.5` | backtest/default path code 기준 |
