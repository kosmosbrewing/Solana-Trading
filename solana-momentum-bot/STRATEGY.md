...
> Updated: 2026-04-12 (Cupsey-Primary 전환 + Paper PnL cost deduction)
...
Volume lookback: 20
Volume multiplier: 2.5           ← 1.3 → 2.5 (2026-04-12): signal quality gate 도입 후 trigger 품질 강화
Min buy ratio: 0.70              ← 0.50 → 0.70 (2026-04-12): 저품질 buy pressure signal 차단
Cooldown: 300s
```

참고:
- 2026-04-12: signal quality gate 도입과 함께 trigger 강화. 이전 baseline `1.3 / 0.50` (04-11 완화)에서 `2.5 / 0.70`으로 역방향 강화.
...
STALK window = 20s (pullback -0.5% 대기, -1.5% 초과 drop = skip)
PROBE window = 45s
...
| `REALTIME_VOLUME_SURGE_MULTIPLIER` | `2.5` | 1.3 → 2.5 (2026-04-12): signal quality gate 도입 후 trigger 품질 강화 |
| `REALTIME_VOLUME_SURGE_LOOKBACK` | `20` | lookback 30보다 우세 |
| `REALTIME_BOOTSTRAP_MIN_BUY_RATIO` | `0.70` | 0.50 → 0.70 (2026-04-12): signal quality gate 도입 후 trigger 품질 강화 |
...
> 2026-04-12: bootstrap_10s **signal-only** (executionRrReject=99.0, volSurge=2.5, buyRatio=0.70) + **Cupsey Lane primary** (STALK→PROBE→WINNER, sweep 최적 파라미터 + Signal Quality Gate, 0.01 SOL, 50-trade edge 검증 중) + KOL Wallet Tracker. Paper PnL에 왕복 비용 차감 도입. Strategy A/C dormant, D sandbox only.
