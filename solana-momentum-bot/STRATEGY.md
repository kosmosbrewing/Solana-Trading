...
> Updated: 2026-04-11 (Mission Pivot A/B 반영)
...
| 전략 | 상태 | 역할 |
|---|---|---|
| Realtime bootstrap `bootstrap_10s` | **active default** | 10초봉 volume+buyRatio 2-gate 트리거 (Option β 파라미터) |
| **Cupsey Lane `cupsey_flip_10s`** | **🆕 active canary** | bootstrap signal 재사용 + post-entry 30-45s quick reject/winner hold state machine. 0.01 SOL fixed ticket. `CUPSEY_LANE_ENABLED=true` |
...
```text
Ticket = 0.01 SOL fixed (risk-per-trade 무관)
PROBE window = 45s
PROBE → WINNER: MFE ≥ +0.1%
PROBE → REJECT: MAE ≤ -1.0% OR 45s timeout
WINNER trailing = 0.5%
WINNER max hold = 5min
...
> 2026-04-11: bootstrap_10s (Option β: backtest 정합 + ATR floor) + **Cupsey Lane** (post-entry quick reject/winner hold, 0.01 SOL canary) + **KOL Wallet Tracker** (cupsey wallet → scanner discovery) 의 3-lane 구조. Strategy A/C dormant, D sandbox only.
