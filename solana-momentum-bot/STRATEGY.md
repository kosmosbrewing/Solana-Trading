...
Gate 2A: Execution Viability — effectiveRR < 1.2 → reject / < 1.5 → size 50%
...
liquiditySize = poolTVL × 2%
emergencySize = TVL 50% 급락에서도 loss ≤ maxRisk

hardCap = portfolio × 20%
```
...
### Age Bucket Graduated Sizing (v2)

| 토큰 나이 | sizeMultiplier |
|----------|----------------|
| < 20min | reject (rug 위험) |
| 20min ~ 2h | × 0.25 |
| 2h ~ 24h | × 0.5 |
...
LP/ownership 감산과 곱셈 누적. `enableAgeBuckets` (default: true).
...
| Cooldown | 3연패 | 30분 대기 |
| Max Concurrent | 1 포지션 (Runner 중 +1 허용, flag) | 추가 진입 차단 |
| Pair Blacklist | 10-trade: WR≤35% + R:R≤1.0 | 자동 차단 |
