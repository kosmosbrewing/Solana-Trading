...
> Updated: 2026-04-14 (Trigger 파라미터 복원 + CUSUM 조기 감지 도입)
...
Volume lookback: 20
Volume multiplier: 1.3           ← 2.5 → 1.3 (2026-04-14): signal drought 복원. gate가 quality 담당
Min buy ratio: 0.50              ← 0.70 → 0.50 (2026-04-14): signal drought 복원. gate가 quality 담당
Cooldown: 300s
```

참고:
- 2026-04-14: 32h 0-trade signal drought → trigger 파라미터 복원. gate(88% reject)가 quality 담당, trigger는 throughput 담당.
...
Signal Quality Gate (cupseyGate):
  ...
  Trade Count Density: 3-bar avg / baseline avg ≥ 1.5x
  env: CUPSEY_GATE_ENABLED (default: true)
```
...
| `REALTIME_VOLUME_SURGE_MULTIPLIER` | `1.3` | 2.5 → 1.3 (2026-04-14): signal drought 복원. gate가 quality 담당 |
| `REALTIME_VOLUME_SURGE_LOOKBACK` | `20` | lookback 30보다 우세 |
| `REALTIME_BOOTSTRAP_MIN_BUY_RATIO` | `0.50` | 0.70 → 0.50 (2026-04-14): signal drought 복원. gate가 quality 담당 |
...
> 2026-04-14: bootstrap_10s **signal-only** (executionRrReject=99.0, volSurge=1.3, buyRatio=0.50) + **Cupsey Lane primary** (STALK→PROBE→WINNER, sweep 최적 파라미터 + Signal Quality Gate + CUSUM observation-only, 0.01 SOL, 50-trade edge 검증 중) + KOL Wallet Tracker. Trigger 파라미터 복원 (signal drought 해소) + CUSUM 조기 감지 도입. Strategy A/C dormant, D sandbox only.
