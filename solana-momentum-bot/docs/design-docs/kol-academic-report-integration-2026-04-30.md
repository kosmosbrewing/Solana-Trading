...
| **A2** | Partial take @ T1 (+40~60% 30~50% 부분 실현) | ❌ RUNNER_T1/T2/T3 모두 full position trail | **보류** — Trade DB schema / wallet ledger 영향 측정 후 별도 sprint |
...
| **2026-05-01** | **Phase 2.A2 P0 — Partial Take @ T1 promote** | `kolHunterPartialTake*` 3 신규 config (default false). T1 promote 시 30% lock-in + 70% runner 잔존. `partialTakeAtSec` marker 재실행 방지. `appendPartialTakeLedger` jsonl. F1 fix (recovery 시 RUNNER_T1+ 의 partialTakeAtSec set) + F2 fix (live position 차단 — wiring 미구현). 회귀 테스트 4건. 학술 §convexity (Taleb / Carver / Moskowitz). 7일 paper Top 10 의 8/10 retreat 70%+ 직접 차단 lever |
