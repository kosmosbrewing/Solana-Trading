...
> 2026-05-14 정합성 주석: 아래 Real Asset Guard 값은 현재 운영 기준으로 갱신됐다.
> `MISSION_CONTROL.md` 가 wallet floor `0.6 SOL`, KOL ticket `0.02 SOL`,
...
| 항목 | 값 | 근거 |
|------|-----|------|
| Wallet floor | 0.6 SOL | `MISSION_CONTROL.md` 2026-05-14 운영 완화 |
...
2026-05-14 운영 기준으로 floor 는 `0.6 SOL` 로 갱신됐다. 원문 사명은
"floor 보호 + 200 live trades + 5x+ winner 실측" 구조로 유지한다.

본 결정 이후:
- 0.6 floor: Real Asset Guard 불변 ✅
