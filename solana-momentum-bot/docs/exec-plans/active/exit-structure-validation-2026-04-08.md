...
> Scope: TP1 partial 30% / TP2 10×ATR / SL 1.5×ATR runner-centric 구조와 `EXHAUSTION / TIME_STOP` 보존 성질의 live 적합성 측정 + 가설 옵션 분기 (**parameter / winner-preservation side**)
> Depends on: [`exit-execution-mechanism-2026-04-08.md`](./exit-execution-mechanism-2026-04-08.md) — Phase X2 v2 finding (`TP2 intent → actual = 0/10`)이 mechanism issue 임이 확인되어 **parameter 결정은** mechanism plan baseline 이후에만 valid. 단, distribution / exhaustion measurement 자체는 병렬 진행 가능
...
- exit mechanism plan 과 **병렬 계측**은 허용된다. 다만 parameter 결정은 mechanism baseline 이후여야 한다
...
| Exhaustion | 최소 10분 후, `currentPrice > entry`면 exit 가능 | runner 전개 전 winner를 잘라먹는지 별도 확인 필요 |
...
- 2026-04-08 최신 clean fresh trade 1건: `effective_rr=6.28` 이었지만 `EXHAUSTION` 손실 종료
...
TP1 / TP2 / SL / TRAILING / TIME_STOP / EXHAUSTION 6가지 종료 사유의 도달 빈도 + 평균 R 분포 측정. 특히 `pre-TP1 EXHAUSTION` 여부를 분리해 exit 구조 결론을 위한 clean baseline 확보.
...
- [ ] **신규 script**: `scripts/analysis/exit-distribution-audit.ts`
  ...
  - 출력 2b: `EXHAUSTION before TP1` / `EXHAUSTION after TP1` 분리
...
- [ ] TP1 도달율 / TP2 도달율 / SL 도달율 / TRAILING 도달율 / TIME_STOP 도달율 / EXHAUSTION 도달율 6개 수치가 명확히 산출됨
- [ ] 평균 R per exit reason이 산출됨 (NaN 방어 — 표본 0건 reason은 명시적 표시)
- [ ] `pre-TP1 EXHAUSTION` 비율이 별도 산출됨
...
### Scenario E — "winner가 TP1 이전 exhaustion으로 잘린다"

**진입 조건**: `EXHAUSTION` 비중이 유의미하고, 그중 다수가 `high_water_mark < take_profit1` 또는 `TP1 미도달` 상태에서 종료

**가설**: 현재 exhaustion guard가 runner 전개 전 winner를 조기 차단

**옵션**:
- **E1**: exhaustion 발동 최소 보유시간 상향
- **E2**: exhaustion은 `TP1 도달 이후` 또는 `최소 미실현 이익 확보 후`에만 허용
- **E3**: exhaustion은 shadow-only 로 먼저 관찰하고 live canary에서 무효화 비교

**검증**: paper mode + live canary 분리 비교
...
|---|---|---|---|
| Phase X1 — Hygiene (clean ≥ 20) | 🟢 통과 (2026-04-10 48h trade-report: clean 24-26 entry groups) | igyubin | 완료 |
| Phase X2 — Distribution Audit | 🟢 n=18 v2 audit + 48h 재진단 — TP2 actual reach 0%, expectancy -0.00108 SOL/trade | igyubin | 완료 |
| Phase X3 — Hypothesis Test | 🟡 **Option β 진행 중 (2026-04-10)** — Scenario A+B+TP1 partial 제거 결합, [`docs/design-docs/strategy-redesign-2026-04-10.md`](../../design-docs/strategy-redesign-2026-04-10.md) 참조 | igyubin | post-deploy ≥ 15 clean trades 후 평가 |
| Phase X4 — Decision Window | 🔴 pending | igyubin | Phase X3 live 전환 후 7일 |

### Phase X3 Option β 선택 근거 (2026-04-10)

48h live trade-report + `BACKTEST.md` 2026-04-01 5m sweep 재검토 결과:

- **Scenario A 확정**: TP2 actual reach = 0/24+ clean groups → TP2 multiplier 10.0 → **5.0** (backtest 100% 수렴값)
- **Scenario B 확정**: TP1 distance 0.05~0.55% 관측 (noise floor 수준) → ATR floor `0.008` (0.8%) 신규 도입
- **추가 (plan scope 확장)**: TP1 partial 30% 제거 (backtest 에 없는 로직이 runner thesis 파괴) → tp1PartialPct 0.3 → **0**
- **보조**: realtimeSlAtrMultiplier 1.5 → 2.0 (swap latency 버퍼), realtimeTimeStopMinutes 15 → 20 (backtest mode)
- **tp1Multiplier 1.0 → 1.5** (backtest mode)

**lock 해제**: 본 Phase X3 진입과 함께 `Forbidden` 절의 orderShape lock 은 Option β 범위 내에서 해제됨. lock 유지: Option β 재 revert 후 또다른 튜닝 금지.

## Forbidden (이 plan scope 안의 금지 사항)

> **2026-04-10 업데이트**: Phase X1 gate 통과 후 Option β 진입으로 `orderShape` / `realtimeSlAtrMultiplier` / `realtimeTimeStopMinutes` / `atrFloorPct` 변경은 **Option β 범위 내에서 1회** 허용됨 (rollback 권한 포함). 아래 금지는 **Option β 배포 후 추가 튜닝 금지** 의 의미로 해석한다.

- Option β 배포 후 추가 `tradingParams.ts` 재튜닝 — 15 clean trades 누적 전 금지
- `config.exhaustionThreshold` 직접 변경 — Scenario E 진입 전 금지
- runner Grade A/B 조건 변경 — Phase X3 결과로도 부족 시 별도 plan 필요
- 표본 < 15일 때 Option β 를 "틀렸다"고 단정하는 것 (rollback 조건은 별도, strategy-redesign-2026-04-10.md §7 참조)
