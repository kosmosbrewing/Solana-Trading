...
> Updated: 2026-04-16 (Cupsey gate soft fail-open: vol_accel 1.5→1.2, buy_ratio 0.55→0.50, price_chg 0.001→0)
...
> Pivot design: [`docs/design-docs/mission-pivot-ab-parallel-2026-04-11.md`](./docs/design-docs/mission-pivot-ab-parallel-2026-04-11.md)
...
| 전략 | 상태 | 역할 |
|---|---|---|
| Realtime bootstrap `bootstrap_10s` | **signal-only** (cupsey-primary) | 10초봉 volume+buyRatio 2-gate 트리거. `executionRrReject=99.0`으로 실거래 100% 억제, signal 생성 전용. rollback: `EXECUTION_RR_REJECT=1.2` |
| **Cupsey Lane `cupsey_flip_10s`** | **active primary** | bootstrap signal 재사용 + STALK→PROBE→WINNER state machine. 0.01 SOL fixed ticket. `CUPSEY_LANE_ENABLED=true` |
...
| Strategy D `new_lp_sniper` | sandbox (live 미연결) | 별도 지갑 실험 전략. sandbox Executor 미구현으로 live 차단 |
| Strategy E `momentum_cascade` | conditional | Strategy A add-on |
| **KOL Wallet Tracker** | **🆕 discovery source** | cupsey wallet buy 감지 → scanner watchlist 자동 추가. `KOL_WALLET_TRACKING_ENABLED=true` |

### 2026-04-11 Mission Pivot 근거

5일간 live 운영 진단 결과:
- 48h clean expectancy: **-0.00108 SOL/trade** (음수)
- bootstrap_10s trigger 는 **lagging indicator** (volume spike 이후 감지 → spike 꼭대기 매수)
- TP2 actual reach: **0%** (전 구간), TP1 fill: 8/8 = 100% loss
- 인프라는 작동 (defense layers, Phase E1 telemetry, ATR floor, PRICE_ANOMALY guard)
- **entry thesis 자체가 문제** → exit/mechanism 튜닝으로 해결 불가

→ **Path A (cupsey-inspired post-entry state machine)** + **Path B (KOL wallet discovery)** 로 pivot.
근거 문서: [`docs/design-docs/mission-pivot-ab-parallel-2026-04-11.md`](./docs/design-docs/mission-pivot-ab-parallel-2026-04-11.md)
...
Volume lookback: 20
Volume multiplier: 1.8           ← VPS 검증 baseline (1.3은 gate와 vol_accel 불일치로 전량 차단)
Min buy ratio: 0.60              ← VPS 검증 baseline
Cooldown: 300s
```

참고:
- 2026-04-14: volSurge=1.3이 vol_accel 중간값 1.17 → gate(1.5) 전량 차단. 1.8/0.60은 vol_accel 중간값 ~2.2였던 검증 baseline.
- bootstrap signal은 `strategy=bootstrap_10s`로 기록되며, cupsey lane의 trigger source로만 사용 (실거래 억제: `executionRrReject=99.0`).
- CUSUM 조기 감지: observation-only 모드로 gate log에 strength 기록 (Phase 1 상관 분석용).
...
### Current Realtime Order Shape (Live Path — Option β 2026-04-10)

```text
SL mode = atr
SL ATR multiplier = 1.25        ← REALTIME_SL_ATR_MULTIPLIER (backtest 수렴값)
TP1 = ATR x 1.5                 ← 1.0 → 1.5 (backtest mode)
TP2 = ATR x 5.0                 ← 10.0 → 5.0 (backtest 100% 수렴)
ATR floor = 0.8% of price       ← 신규: 10s noise floor 보정
TP1 partial = 0% (full close)   ← 0.3 → 0 (partial 제거)
Trailing after TP1 = false       ← entry 직후 trailing 가능
Time Stop = 20m                  ← 15 → 20 (backtest mode)
Nominal RR = 5.0 / 1.25 = 4.0
```

**Effective min distances (ATR floor)**: TP1 ≥ +1.2%, TP2 ≥ +4.0%, SL ≥ -1.0%

### Cupsey Lane Order Shape (Path A — 2026-04-12 sweep 최적 반영)

bootstrap_10s signal 재사용, post-entry state machine 으로 exit 관리.
Signal Quality Gate + sweep 최적 파라미터 반영 (2026-04-12).
...
WINNER trailing = 4.0%
WINNER max hold = 12min
WINNER breakeven = entry + 0.5%
Max concurrent = 5 (config: cupseyMaxConcurrent)

Signal Quality Gate (cupseyGate):
  Volume Acceleration: 3-bar avg / baseline avg ≥ 1.2x  ← 1.5 → 1.2 (2026-04-16): 측정 우선 전략
  Price Momentum: 3-bar price change ≥ 0%              ← 0.001 → 0 (2026-04-16): STALK은 pullback 진입, 하락 중 신호도 적합
  Buy Ratio Consistency: 3-bar avg buy ratio ≥ 0.50    ← 0.55 → 0.50 (2026-04-16)
  Trade Count Density: 3-bar avg / baseline avg ≥ 1.0x  ← 1.5 → 1.0 (2026-04-14)
  env overrides: CUPSEY_GATE_ENABLED / CUPSEY_GATE_MIN_VOLUME_ACCEL_RATIO /
                 CUPSEY_GATE_MIN_PRICE_CHANGE_PCT / CUPSEY_GATE_MIN_AVG_BUY_RATIO /
                 CUPSEY_GATE_MIN_TRADE_COUNT_RATIO

CUSUM 조기 감지 (Phase 0, observation-only):
  cusumKMultiplier=0.3, cusumHMultiplier=4.0, warmupPeriods=10
  → gate log에 strength 기록만. trade 결정 영향 없음. Phase 1 상관 분석 예정.
```
...
| `REALTIME_SL_ATR_MULTIPLIER` | — | `1.25` | 2026-04-10: 1.5 → 1.25 (backtest 수렴값. ATR floor 0.8%가 noise 흡수) |
...
| `EXECUTION_RR_REJECT` | `99.0` (cupsey-primary: bootstrap 실거래 억제. rollback → `1.2`) |
...
| `REALTIME_VOLUME_SURGE_MULTIPLIER` | `1.8` | VPS 검증 baseline. 1.3은 vol_accel 중간값 1.17로 gate 전량 차단됨 |
| `REALTIME_VOLUME_SURGE_LOOKBACK` | `20` | lookback 30보다 우세 |
| `REALTIME_BOOTSTRAP_MIN_BUY_RATIO` | `0.60` | VPS 검증 baseline |
...
| `SL_ATR_MULTIPLIER` | `1.25` | runtime_canary. code_default: `1.0`, **live_path: `1.25`** (Option β 2026-04-10) |
...
> 2026-04-16: bootstrap_10s **signal-only** (executionRrReject=99.0, volSurge=1.8, buyRatio=0.60) + **Cupsey Lane primary** (gate soft fail-open: vol_accel≥**1.2**, buy_ratio≥**0.50**, price_chg≥**0** + CUSUM observation-only, 0.01 SOL 측정 우선, 20-30 trade 블록 후 재평가). executed-buys/sells fallback ledger + integrity halt + execution funnel stats 추가. Strategy A/C dormant, D sandbox only.
