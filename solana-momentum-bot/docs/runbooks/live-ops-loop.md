...
- `trade-report-latest.txt`와 `realized-replay-ratio-latest.md`는 항상 현재 분석 window와 정확히 일치하지 않을 수 있다.
- runtime은 이미 새 세션으로 진행 중인데 `results/vps-analysis/*-latest`가 이전 loop 시각에 멈춰 있으면, synced snapshot과 수동 same-window report를 분리해서 기록한다.

이 명령은 아래를 빠르게 보여준다.

- opened row / closed row / open row / partial close row
- realized pnl / win-loss / token count
- entry-group 기준 `EXHAUSTION (entry): N건 | pre-TP1 X | post-TP1 Y`
- 종목별 종료 사유 / 보유 시간
...
- **Phase M day-2부터** `metrics_note`에 다음 5종을 1줄씩 기록한다 (`live-ops-integrity-2026-04-07.md` Phase M acceptance):
  ...
  5. `pre_tp1_exhaustion_entries_recent_${H}h` — `trade-report.ts --hours ${H}` 출력의 `EXHAUSTION (entry): N건 | pre-TP1 X | post-TP1 Y` 라인에서 추출. incident drill-down에서는 `scripts/analysis/exit-distribution-audit.ts --closed-after <ISO>`로 same-window 재검증 가능하다.
- 위 5종이 metrics_note에서 빠지면 Phase M 7일 acceptance에 자동으로 미달이다. 7일 누적이 `entry_gap p95 ≤ 5%` 합격이 안 나오면 Phase C2 raw 합격이 영구 실패 상태이므로 fake-fill threshold 또는 size sanitizer 보강을 우선 검토. `pre_tp1_exhaustion_entries_recent_${H}h > 0`가 반복되면 fake-fill 이슈와 별개로 `winner preservation` 축을 별도 P0로 올린다.
...
- executed mint concentration
- low-cap / high volume-mcap 후보가 실제로 execution까지 갔는지
- execution failed가 특정 mint에만 몰리는지
- 실제 executed entry가 손익 기준으로도 살아 있는지 (`row`와 `entry` 둘 다)
- `pre-TP1 EXHAUSTION`이 발생했는지, 발생했다면 fresh / mature 어느 코호트에 몰리는지
...
또한 signal이 충분해도 `execution_failed`가 대량이면 trigger 부족이 아니라 execution integrity가 주병목일 수 있다.
그리고 executed entry 수가 적더라도 `pre-TP1 EXHAUSTION`이 보이면 exit mechanism보다 먼저 `winner가 TP1 이전에 잘리는지`를 따로 본다.
...
### 7) `execution_failed` 지배 + `PRICE_ANOMALY_BLOCK` 반복

우선 확인:

- `execution_failed` count와 top pair 분포
- `PRICE_ANOMALY_BLOCK`의 entry ratio 분포
- anomaly가 특정 ticker / decimals / fill path에만 몰리는지
- anomaly를 제외한 실제 executed entry의 손익 (`row` / `entry`)
- latest `trade-report`가 현재 runtime window와 같은 시각대인지

원칙:

- 이 경우는 guard 자체를 끄는 문제가 아니라, guard가 계속 발화하는 root cause를 먼저 찾는다
- runtime status는 `execution_failed`지만, 해석은 `guard 정상 동작 + upstream price alignment 불량`으로 분리한다
- anomaly가 많은 상태에서는 `cooldown`이나 `volume multiplier`를 먼저 건드리지 않는다
- anomaly를 제외한 executed entry도 연속 손실이면 execution issue와 별도로 edge / exit 구조를 다시 본다

### 8) `pre-TP1 EXHAUSTION > 0`

우선 확인:

- `trade-report.ts --hours <H>`의 `EXHAUSTION (entry)` 라인
- `scripts/analysis/exit-distribution-audit.ts --closed-after <ISO>`로 same-window 재검증
- `effective_rr`, `take_profit1`, `high_water_mark`, `hold time`
- `fresh / mid / mature` 코호트 중 어디에 몰리는지

원칙:

- 이 경우는 `TP2 미체결`보다 먼저 `winner preservation` 문제로 읽는다
- `TP1/TP2` 파라미터를 바로 바꾸기 전에 `EXHAUSTION before TP1`가 구조 문제인지 data quality인지 먼저 분리한다
- 반복되면 `exit mechanism P0-A`와 병렬로 `winner preservation audit P0-C`를 올린다
