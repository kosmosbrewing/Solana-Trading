...
3. incident성 판단이면 `npx ts-node scripts/trade-report.ts --hours <N>`를 실행해 `db_window`, `db_commands`, `db anomalies`를 채운다
4. Codex 답변 후 `assessment`, `action`, `next check`를 확정한다
